...
type Internals = {
  ...
  createOrderTransaction: unknown;
};
...
const createOrder = (payloadOverrides: Partial<CreateOrderPayload> = {}) =>
  internals.createOrderCore(params, payload(payloadOverrides), options);

let transaction: ReturnType<typeof vi.fn>;

beforeEach(() => {
  transaction = vi.fn();
  internals.createOrderTransaction = transaction;
...
describe("OrdersService.createOrderCore — 주문번호 순번 충돌", () => {
  ...
  it("재시도를 소진하면 인덱스 이름 대신 ORDER_NUMBER_CONFLICT를 던진다", async () => {
    ...
    const error = await createOrder().then(
      () => null,
